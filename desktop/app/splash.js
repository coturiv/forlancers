 ...
 * @param {string} splashOptions.imageFileName Name of file placed in resources/splash folder
 ...
 */
  ...
  init() {
    ...
    let imagePath = path.join(rootPath, 'resources/splash', this.splashOptions.imageFileName);
    let imageUrl = '';
    ...
    if (useFallback) {
      this.splashWindow.loadURL(`data:text/html;charset=UTF-8,${splashHtml}`, {baseURLForDataURL: `file://${rootPath}/resources/splash/`});
    } else {
    ...
    this.splashWindow.webContents.on('dom-ready', async () => {
      ...
      setTimeout(async () => {
        this.mainWindowRef.loadURL(`file://${rootPath}/www/index.html`);
      }, 4500);
    });
